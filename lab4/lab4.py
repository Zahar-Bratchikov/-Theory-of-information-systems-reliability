...
P_substitute = 0
mult = 1
for i, Pp in enumerate(P_switches): # Pp - вероятность срабатывания переключателя
    term = mult * Pp * P1 # term - это добавка к надёжности от i-го резервного устройства, если оно смогло подключиться
    P_substitute += term # P_substitute - накапливает вероятность того, что хотя бы один резерв сработает
    mult *= (1 - Pp) # mult — вероятность, что все предыдущие переключатели не сработали
